...
export async function applyToGuildApplicationViaUI(
  ...
  for (let step = 0; step < 24; step++) {
    ...
    if (await submit.isVisible().catch(() => false)) {
      ...
      await submit.click();
      await expect(page.getByText(/application submitted|application received|submitted/i).first()).toBeVisible({
        timeout: 30_000,
      });
  ...
  throw new Error("applyToGuildApplicationViaUI: exhausted form steps before submit");
}
...
export async function findAssignedExperts(
  ...
  for (const expert of args.experts) {
    ...
    if (!res.ok()) {
      throw new Error(`findAssignedExperts: list failed ${res.status()} ${await res.text()}`);
    }
...
export async function submitCandidateReviewViaUI(
  ...
  await expect(page.getByText(/review candidate application/i).first()).toBeVisible();
...
export async function openCandidateReviewFromGuildQueueViaUI(
  ...
  const reviewLink = page
    .locator(`a[href*="reviewAppId=${args.applicationId}"][href*="reviewType=candidate"]`)
    .first();
  await expect(reviewLink).toBeVisible({ timeout: 30_000 });
  await reviewLink.click();
  await expect(page).toHaveURL(new RegExp(`/expert/voting\\?[^#]*reviewAppId=${args.applicationId}`));
  await expect(page.getByLabel("Close review modal").first()).toBeVisible({
    timeout: 30_000,
  });
  await expect(page.getByText(/review candidate application/i).first()).toBeVisible();
...
export async function expectSubmittedReviewInMyReviewsViaUI(
  ...
  await page.goto(`/expert/guild/${encodeURIComponent(args.guildId)}?tab=reviews`, {
    waitUntil: "domcontentloaded",
  });
  const reviewLink = page
    .locator(`a[href*="reviewAppId=${args.applicationId}"][href*="reviewType=candidate"]`)
...
export async function completeCandidateReviewModalViaUI(
  ...
  for (let step = 0; step < 24; step++) {
    ...
    if (await submit.isVisible().catch(() => false)) {
      ...
      await submit.click();
      await expect(
        page.getByText(/review submitted|done/i).or(page.getByRole("button", { name: /^done$/i })).first(),
  ...
  throw new Error("submitCandidateReviewViaUI: exhausted review steps before submit");
}

async function completeVisibleCandidateApplySubstep(page: Page): Promise<void> {
  const fileInput = page.locator('input[type="file"]').first();
  if (await fileInput.count()) {
    const uploadButton = page.getByRole("button", { name: /upload a new resume/i });
    if (await uploadButton.isVisible().catch(() => false)) {
      ...
      await expect(page.getByText(/uploaded|e2e-resume\.pdf/i).first()).toBeVisible({
        timeout: 20_000,
      });
    }
  }

  const experienced = page.getByRole("button", { name: /experienced/i }).first();
  if (await experienced.isVisible().catch(() => false)) {
    await experienced.click();
  }

  const attestation = page.locator("label", { hasText: /i wrote this myself/i }).first();
  ...
  for (let i = 0; i < count; i++) {
    ...
    if ((await textarea.isVisible().catch(() => false)) && (await textarea.inputValue()) === "") {
      await textarea.fill(CANDIDATE_ANSWER);
    }
  }
}

async function completeVisibleReviewSubstep(page: Page, preferredScore: number): Promise<void> {
  const generalCard = page.locator('[id^="review-field-general-"]:visible').first();
  ...
  const domainCard = page.locator('[id^="review-field-domain-"]:visible').first();
...
async function clickMaxScoreButtons(
  ...
  await scope.evaluate((root, score) => {
    const groups = Array.from(root.querySelectorAll("div"))
      .map((el) => Array.from(el.children).filter((child) => child instanceof HTMLButtonElement) as HTMLButtonElement[])
      .filter((buttons) => {
        const labels = buttons.map((button) => button.textContent?.trim() ?? "");
        return labels.length > 1 && labels[0] === "0" && labels.every((label) => /^\d+$/.test(label));
      });

    for (const buttons of groups) {
      const numeric = buttons
        .map((button) => ({ button, value: Number(button.textContent?.trim() ?? "0") }))
...
async function fetchLatestCandidateGuildApplicationId(page: Page): Promise<string> {
  const token = await page.evaluate(() => window.localStorage.getItem("authToken"));
  if (!token) throw new Error("fetchLatestCandidateGuildApplicationId: missing candidate auth token");

  const res = await page.request.get(`${BACKEND_URL}/api/candidates/me/guild-applications`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  if (!res.ok()) {
    throw new Error(`fetchLatestCandidateGuildApplicationId: list failed ${res.status()} ${await res.text()}`);
  }
  const body = (await res.json()) as { data: Array<{ id: string; createdAt?: string; created_at?: string }> };
  const latest = body.data[0];
  if (!latest?.id) throw new Error("fetchLatestCandidateGuildApplicationId: no applications returned");
  return latest.id;
}
