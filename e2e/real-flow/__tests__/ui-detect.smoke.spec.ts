...
test("EIP-6963 announce reaches the page after navigation", async ({
  page,
}) => {
  await test.step("Verify: EIP-6963 announce reaches the page after navigation", async () => {
...
test("opening the connect modal shows an Installed section with our shim", async ({
  page,
}) => {
  await test.step("Verify: opening the connect modal shows an Installed section with our shim", async () => {
    await attachWallet(page, ACCOUNT_1_KEY, { rpcUrl: RPC });
    await page.goto("/auth/login?type=expert");
    await page.waitForLoadState("networkidle");
