...
test("candidate applies through UI -> assigned experts review through UI -> rewards and reputation update", async ({
  ...
  const reviewPanel: Expert[] = assignedExperts.slice(0, majorityThreshold);

  const beforeProfiles = await Promise.all(
    reviewPanel.map((expert) => fetchExpertProfile(page, expert.address)),
  );

  for (let i = 0; i < reviewPanel.length; i++) {
    const reviewer = reviewPanel[i];
    await reviewApplicationAsExpert(page, reviewer, reviewFixture.guild.id, applicationId, {
      viaWorkspaceQueue: i === 0,
      verifyMyReviews: i === 0,
    });
    ...
    expect(mine.data.overall_score ?? mine.data.overallScore ?? 0).toBeGreaterThan(0);
  }

  const application = await fetchCandidateGuildApplication(page, candidate.token, applicationId);
  expect(application.status).toBe("approved");

  for (let i = 0; i < reviewPanel.length; i++) {
    const afterProfile = await fetchExpertProfile(page, reviewPanel[i].address);
    expect(afterProfile.reputation).toBeGreaterThan(beforeProfiles[i].reputation);

    const earnings = await fetchExpertEarnings(page, reviewPanel[i].address, reviewFixture.guild.id);
...
test("candidate review deadline finalization penalizes missed reviewers", async ({
  ...
  expect(assignedExperts.length, "slashing flow needs one reviewer and one missed reviewer").toBeGreaterThan(1);

  const reviewer = assignedExperts[0];
  const missedReviewers = assignedExperts.slice(1);
  await reviewApplicationAsExpert(page, reviewer, reviewFixture.guild.id, applicationId);

  const state = await testApi.candidateReviews.expireAndFinalize(page.request, applicationId);
  expect(state.finalization).toMatchObject({ processed: 1, succeeded: 1, failed: 0 });
  expect(state.application.status).toBe("approved");

  const reviewedAssignment = state.assignments.find(
    (row) => row.wallet_address.toLowerCase() === reviewer.address.toLowerCase(),
  );
  expect(reviewedAssignment).toMatchObject({ has_reviewed: true, forfeited: false });

  for (const missed of missedReviewers) {
    const missedAssignment = state.assignments.find(
      (row) => row.wallet_address.toLowerCase() === missed.address.toLowerCase(),
    );
    expect(missedAssignment).toMatchObject({ has_reviewed: false, forfeited: true });
...
test("candidate review deadline outcome rewards aligned reviewers and penalizes dissenting reviewers", async ({
  ...
  expect(assignedExperts.length, "misalignment flow needs at least two reviewers").toBeGreaterThanOrEqual(2);
  ...
  await reviewApplicationAsExpert(page, dissenting, reviewFixture.guild.id, applicationId, { score: 5 });
  await reviewApplicationAsExpert(page, aligned, reviewFixture.guild.id, applicationId, { score: 1 });
  ...
  const alignedEarnings = await fetchExpertEarnings(page, aligned.address, reviewFixture.guild.id);
  expect(alignedEarnings.summary.totalVetd).toBeGreaterThan(0);

  const dissentLog = state.reputationLog.find(
    (row) => row.expert_id === dissenting.id && row.event_type === "misaligned",
  );
  ...
  const dissentingProfile = await fetchExpertProfile(page, dissenting.address);
  expect(dissentingProfile.reputation).toBe(beforeDissenting.reputation - 5);

  const dissentingEarnings = await fetchExpertEarnings(page, dissenting.address, reviewFixture.guild.id);
...
async function reviewApplicationAsExpert(
  ...
  opts: { viaWorkspaceQueue?: boolean; verifyMyReviews?: boolean; score?: number } = {},
): Promise<void> {
  ...
  try {
    ...
    if (opts.viaWorkspaceQueue) {
      await openCandidateReviewFromGuildQueueViaUI(reviewPage, { guildId, applicationId });
    ...
    if (opts.verifyMyReviews) {
      await expectSubmittedReviewInMyReviewsViaUI(reviewPage, { guildId, applicationId });
    }
  } finally {
...
async function fetchCandidateGuildApplication(
  ...
  const res = await page.request.get(`${BACKEND_URL}/api/candidates/me/guild-applications`, {
...
async function fetchExpertProfile(
  ...
  const body = (await res.json()) as {
    data: { reputation?: number; reputationScore?: number; reputation_score?: number };
  };
  return {
    reputation: Number(body.data.reputation ?? body.data.reputationScore ?? body.data.reputation_score ?? 0),
  };
}
...
async function fetchExpertEarnings(
  ...
  expect(res.ok(), await res.text()).toBeTruthy();
  const body = (await res.json()) as { data: { summary: { totalVetd: number } } };
  return body.data;
}
