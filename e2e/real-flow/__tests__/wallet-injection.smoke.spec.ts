...
const BLANK_PAGE =
  "data:text/html,<!doctype html><html><body><h1>shim</h1></body></html>";

test("page sees window.ethereum after attachWallet", async ({ page }) => {
  await test.step("Verify: page sees window.ethereum after attachWallet", async () => {
    ...
    const isMetaMask = await page.evaluate(
      () =>
        (window.ethereum as { isMetaMask?: boolean } | undefined)
          ?.isMetaMask === true,
    );
...
test("page-side eth_accounts routes to Node and returns active address", async ({
  page,
}) => {
  await test.step("Verify: page-side eth_accounts routes to Node and returns active address", async () => {
    await attachWallet(page, ACCOUNT_1_KEY, { rpcUrl: RPC });
    await page.goto(BLANK_PAGE);
...
test("page-side personal_sign produces signature recoverable to injected account", async ({
  ...
  await test.step("Verify: page-side personal_sign produces signature recoverable to injected account", async () => {
    await attachWallet(page, ACCOUNT_1_KEY, { rpcUrl: RPC });
    await page.goto(BLANK_PAGE);
...
test("switchAccount propagates new address to page via accountsChanged", async ({
  page,
}) => {
  await test.step("Verify: switchAccount propagates new address to page via accountsChanged", async () => {
    ...
    expect(seen[seen.length - 1].toLowerCase()).toBe(
      ACCOUNT_2_ADDR.toLowerCase(),
    );
...
test("EIP-6963 announce event responds to requestProvider", async ({
  page,
}) => {
  await test.step("Verify: EIP-6963 announce event responds to requestProvider", async () => {
    await attachWallet(page, ACCOUNT_1_KEY, { rpcUrl: RPC });
    await page.goto(BLANK_PAGE);
