...
test("inspect wagmi connectors after load", async ({ page }) => {
  await test.step("Verify: inspect wagmi connectors after load", async () => {
    ...
    const diag = await page.evaluate(async () => {
      ...
      return {
        ...
        hasHwProvider: !!(window as unknown as { __hwProvider?: unknown })
          .__hwProvider,
        hasHwRequest:
          typeof (window as unknown as { __hwRequest?: unknown })
            .__hwRequest === "function",
      };
    });
...
test("manually trigger connect via window.ethereum, then check wagmi useAccount", async ({
  ...
  await test.step("Verify: manually trigger connect via window.ethereum, then check wagmi useAccount", async () => {
    await attachWallet(page, ACCOUNT_1_KEY, { rpcUrl: RPC });
    await page.goto("/auth/login?type=expert");
    await page.waitForLoadState("networkidle");
