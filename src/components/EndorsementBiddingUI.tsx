    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-gradient-to-br from-violet-50 to-indigo-50 rounded-xl p-6 border border-violet-200">
          <div className="flex items-center justify-between mb-2">
            <p className="text-sm font-medium text-violet-700">Your Balance</p>
            <Coins className="w-5 h-5 text-violet-600" />
          </div>
          <p className="text-3xl font-bold text-slate-900">{expertTokenBalance} VETD</p>
          <p className="text-xs text-slate-600 mt-1">Available for bidding</p>
        </div>
        ...
        <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-xl p-6 border border-amber-200">
          ...
          <p className="text-3xl font-bold text-slate-900">{expertReputation}</p>
          <p className="text-xs text-slate-600 mt-1">Affects reward multiplier</p>
        </div>
        ...
        <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-xl p-6 border border-emerald-200">
          ...
          <p className="text-xs text-slate-600 mt-1">Currently staked</p>
        </div>
      </div>
      ...
      <div className="bg-blue-50 rounded-xl p-6 border border-blue-200 mb-8">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="font-semibold text-slate-900 mb-2">How Endorsement Bidding Works</h3>
      ...
      <div>
        <h2 className="text-2xl font-bold text-slate-900 mb-6">Available Candidates</h2>
        {applications.length === 0 ? (
          <div className="text-center py-12 bg-white rounded-xl border border-slate-200">
            <Users className="w-16 h-16 text-slate-300 mx-auto mb-4" />
            <p className="text-slate-500">No applications available for endorsement</p>
          </div>
        ) : (
          <div className="space-y-6">
            {applications.map((app) => (
              <div
                key={app.id}
                className="bg-white rounded-xl p-6 border border-slate-200 hover:border-violet-300 transition-all"
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-slate-500">{app.applicationSummary}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-6">
                    <div>
                      <p className="text-xs text-slate-500 mb-1">Potential Reward</p>
                      <p className="text-lg font-bold text-emerald-600">
                        {calculatePotentialReward(app)} VETD
                      </p>
                    </div>
                    <div>
                      <p className="text-xs text-slate-500 mb-1">To Beat</p>
        ...
        {selectedApplication && (
          <div className="space-y-4">
            <div className="bg-violet-50 rounded-lg p-4 border border-violet-200">
              ...
              <p className="text-xs text-slate-600">{selectedApplication.jobTitle}</p>
            </div>
            ...
            <div className="bg-slate-50 rounded-lg p-4 space-y-2">
              <div className="flex justify-between items-center">
                <p className="text-sm text-slate-600">If hired (reward):</p>
                <p className="text-sm font-bold text-emerald-600">
                  +{calculatePotentialReward(selectedApplication)} VETD
                </p>
              </div>
              <div className="flex justify-between items-center">
                <p className="text-sm text-slate-600">If rejected (slashing):</p>
