...
export default function VotingPage() {
  ...
  const loadProposals = async () => {
    try {
      ...
      if (filterMode === "assigned" && expertData?.id) {
        const response: any = await proposalsApi.getAssignedProposals(expertData.id, selectedGuild.id);
        if (response.success) {
          setProposals(response.data);
        }
      } else {
        // Fetch all proposals for the guild
        const response: any = await proposalsApi.getGuildProposals(selectedGuild.id, "ongoing");
        if (response.success) {
          setProposals(response.data);
        }
      }
  ...
  const handleCreateProposal = async (data: StructuredProposalData) => {
    try {
      const response: any = await proposalsApi.createProposal({
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        <h3
                          ...
                          onClick={() => window.location.href = `/expert/voting/proposals/${proposal.id}`}
                        >
